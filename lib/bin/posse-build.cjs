...
const copyStaticAssets = (folderPath, buildFolder) => __awaiter(void 0, void 0, void 0, function* () {
    console.log(`Copying files from "${folderPath}" to build folder "${buildFolder}"...`);
    const files = yield glob.glob(`${folderPath}/**/*.{${[...supportedImageTypes, ...supportedFontTypes, ...supportedBrowserFiles]}}`);
...
function gallery(indexContents, filePath, buildFolder) {
    return __awaiter(this, void 0, void 0, function* () {
        const folderPath = filePathToFolderPath(filePath);
        copyStaticAssets(folderPath, buildFolder);
        console.log(`Building "${filePath}"...`);
        const images = (yield glob.glob(path.join(folderPath, 'images', `**/*.{${supportedImageTypes.join(',')}}`))).reverse();
...
const FRAGMENTS_PATH = path.join(PROJECT_ROOT, 'scaffolding', 'fragments');
...
const getArticlePaths = (folderPath) => __awaiter(void 0, void 0, void 0, function* () { return yield glob.glob(path.join(folderPath, '*.md')); });
const process$1 = (indexContents, filePath, buildFolder) => __awaiter(void 0, void 0, void 0, function* () {
    const folderPath = filePathToFolderPath(filePath);
    yield copyStaticAssets(folderPath, buildFolder);
    const articlePaths = yield getArticlePaths(folderPath);
    console.log('Building article pages and folders...');
    const articleLinks = buildArticles(articlePaths, folderPath, indexContents, buildFolder);
...
const buildArticleListPage = (filePath, articleLinks, indexContents, buildFolder) => {
    const articleListElements = articleLinks.map((articleLink) => {
        ...
        const aElement = makeElement('a', { class: 'article-link', href: path.join(getHostPath(), urlFriendlyPath) }, articleLink.title);
...
const buildArticles = (articlePaths, folderPath, indexFileContents, buildFolder) => {
    const articleLinks = [];
...
function noTreatment(indexContents, filePath, buildFolder) {
    return __awaiter(this, void 0, void 0, function* () {
        const folderPath = filePathToFolderPath(filePath);
        yield copyStaticAssets(folderPath, buildFolder);
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const indexHtmlFiles = yield glob.glob(indexMatcher, {
        ...
        for (let i = 0; i < indexHtmlFiles.length; i++) {
            ...
            switch (pageType) {
                case 'galleryPage': {
                    ...
                    yield gallery(formattedPage, filePath, BUILD_FOLDER);
                    break;
                }
                case 'articlesList': {
                    console.log(`'pageType' found: 'articlesList' type`);
                    yield process$1(formattedPage, filePath, BUILD_FOLDER);
                    break;
                }
                case 'noTreatment': {
                    console.log(`'pageType' not defined; no treatment applied`);
                    yield noTreatment(formattedPage, filePath, BUILD_FOLDER);
                    break;
                }
            }
        }
