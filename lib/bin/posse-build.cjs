...
const injectPageCSSRelativeLocation = (page, relativeLocation = '.') => {
    return page.replace('{cssPath}', relativeLocation);
};
const formatBaselinePage = (pageContent, relativePageCSSPath) => {
    return injectPageCSSRelativeLocation(injectHostPath(injectFooter(injectHeader(pageContent))), relativePageCSSPath);
};
...
function gallery(indexFileContents, filePath, buildFolder, siteFolder) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const newIndexContents = formattedPage.replace('{galleryContainer}', galleryHTML);
...
const process$1 = (unformattedIndexContents, filePath, buildFolder, siteFolder) => __awaiter(void 0, void 0, void 0, function* () {
    const folderPath = filePathToFolderPath(filePath);
    yield copyStaticAssets(path.join(folderPath, 'css'), buildFolder, siteFolder);
    const articlePaths = yield getArticlePaths(folderPath, siteFolder);
    console.log('Building article pages and folders...');
    const articleLinks = buildArticles(articlePaths, folderPath, unformattedIndexContents, buildFolder, siteFolder);
    buildArticleListPage(filePath, articleLinks, unformattedIndexContents, buildFolder);
});
const buildArticleListPage = (articleListPath, articleLinks, indexFileContents, buildFolder) => {
    ...
    console.log(`Adding header and footer to "${articleListPath}" and setting host path...`);
    const formattedPage = formatBaselinePage(indexFileContents);
    const indexContentsWithLinks = formattedPage.replace(listToken, ul);
    writeFileWithDirectory(path.join(buildFolder, articleListPath), indexContentsWithLinks);
};
const buildArticles = (articlePaths, folderPath, indexFileContents, buildFolder, siteFolder) => {
    ...
    articlePaths.forEach((articlePath) => {
        ...
        console.log(`Adding header and footer to "${articlePath}" and setting host path...`);
        const formattedPage = formatBaselinePage(indexFileContents, '../../..');
        console.log(`Converting file at ${articlePath} to HTML...`);
        const markedUpContent = converter.makeHtml(article);
        const newPageWithMarkedUpContent = formattedPage.replace(listToken, makeElement('div', { class: 'article' }, markedUpContent));
...
function noTreatment(indexFileContents, filePath, buildFolder, siteFolder) {
    return __awaiter(this, void 0, void 0, function* () {
        const folderPath = filePathToFolderPath(filePath);
        yield copyStaticAssets(path.join(folderPath, 'css'), buildFolder, siteFolder);
        yield copyStaticAssets(path.join(folderPath, 'images'), buildFolder, siteFolder);
        ...
        for (let i = 0; i < indexHtmlFiles.length; i++) {
            ...
            switch (pageType) {
                case 'galleryPage': {
                    ...
                    yield gallery(fileContents, indexFilePath, BUILD_FOLDER, SITE_FOLDER);
                    break;
                }
                case 'articlesList': {
                    console.log(`'pageType' found: 'articlesList' type`);
                    yield process$1(fileContents, indexFilePath, BUILD_FOLDER, SITE_FOLDER);
                    break;
                }
                case 'noTreatment': {
                    console.log(`'pageType' not defined; no treatment applied`);
                    yield noTreatment(fileContents, indexFilePath, BUILD_FOLDER, SITE_FOLDER);
                    break;
                }
            }
        }
