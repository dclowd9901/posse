...
const posseNodeModulePath = path.join(process.env.INIT_CWD, 'node_modules', '@dclowd9901', 'posse');
...
try {
    gitignoreContent = fse__namespace.readFileSync(path.join(process.env.INIT_CWD, '.gitignore'), { encoding: 'utf-8' });
}
catch (e) {
}
gitignoreContent += `
...
public
`;
fse__namespace.writeFileSync(path.join(process.env.INIT_CWD, '.gitignore'), gitignoreContent);
