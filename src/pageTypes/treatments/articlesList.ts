...
const getArticlePaths = async (folderPath: string) => await glob(path.join(folderPath, '*.md'));

const process = async (indexContents: string, filePath: string, buildFolder: string) => {
  const folderPath = filePathToFolderPath(filePath);

  await copyStaticAssets(folderPath, buildFolder);

  const articlePaths = await getArticlePaths(folderPath);
...
const buildArticleListPage = (filePath: string, articleLinks: ArticleLinkDescription[], indexContents: string, buildFolder: string) => {
	const articleListElements = articleLinks.map((articleLink: ArticleLinkDescription) => {
...
const buildArticles = (articlePaths: string[], folderPath: string, indexFileContents: string, buildFolder: string): ArticleLinkDescription[] => {
